...
public enum GlassButtonStyle {
    ...
    var glassTint: Color {
        switch self {
        case .primary:
            return .themePrimary.opacity(0.3)
        case .secondary:
            return DesignSystem.tokens.glass.tint
        case .destructive:
            return .error.opacity(0.25)
        }
    }
}
...
#Preview("Glass Buttons") {
    VStack(spacing: DSSpacing.md) {
        GlassButton(title: "Send Reminder") { }
        GlassButton(title: "Skip", style: .secondary) { }
        GlassButton(title: "Delete", style: .destructive) { }
    }
    .padding()
